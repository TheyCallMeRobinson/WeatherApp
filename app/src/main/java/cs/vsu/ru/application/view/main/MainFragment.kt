...
class MainFragment : Fragment() {
    ...
    private fun setDailyWeatherList(dailyWeatherList: List<DailyWeather>) {
        ...
        for (item in dailyWeatherList) {
            ...
            tableRowViewBinding.dayOfWeek.text = DateFormat.format("EEEE", Date(item.date * 1000L)).toString()
            tableRowViewBinding.humidityHolder.itemHumidity.text = HumidityModel(item.humidity).toString()
            tableRowViewBinding.itemDayTemperature.text = TemperatureModel(item.temperature.minTemperature).toString()
            tableRowViewBinding.itemNightTemperature.text = TemperatureModel(item.temperature.maxTemperature).toString()
